...
export default class BrowserSelection {
  ...
  constructor(query, from) {
    ...
    const opts = from ? { path: from } : {};
    this.#list = browserslist(this.browsersRequest, opts).map((browser) => browser.split(' '));
  }
   ...
   * @param {ConstructorParameters<typeof this>} args
   * @return {MissingSupportResult} `{browsers, features}`
   */
  static missingSupport(...args) {
    const selection = new BrowserSelection(...args);
    return {
