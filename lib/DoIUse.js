...
export default class DoIUse {
  ...
  postcss(css, result) {
    ...
    return detector.process(css, ({ feature, usage, ignore }) => {
      ...
      if (this.ignoreOptions?.includes(feature)) {
        return;
      }
      ...
      if (this.onFeatureUsage) {
        ...
        const mappedStart = css.source?.input.map?.consumer().originalPositionFor(start);

        const file = mappedStart?.source ?? input.file ?? input.from;
        const line = mappedStart?.line ?? start.line;
        const column = mappedStart?.column ?? start.column;
